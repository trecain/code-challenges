...
const forLoopTwoToThe = (input) => {
  // Solution code here...
  let numbersRaisedToTwo = [];
  for(let inputs of input) {
    numbersRaisedToTwo.push(Math.pow(2, inputs));
  }
  return numbersRaisedToTwo;
};
...
const forEachTwoToThe = (input) => {
  ...
  let numbersRaisedToTwo = [];
  input.forEach(el => {
    numbersRaisedToTwo.push(Math.pow(2, el));
  });
  return numbersRaisedToTwo;
};
...
const mapTwoToThe = (input) => input.map(element => Math.pow(2, element));
...
const charCode = (input) => input.map(char => char.charCodeAt(0));
// ------------------------------------------------------------------------------------------------
// CHALLENGE 5
//
...
const evenOdd = (input) => input.map(el => isNaN(el) ? 'N/A' : el % 2 === 0 ? 'even' : 'odd');
...
const extractAbilities = abilities => abilities.map(el => el.ability.name);
...
const extractStats = stats => stats.map(stat => ({ name: stat.stat.name, total: stat.effort + stat.baseStat }));
